...
export default function ExamenDocumento() {
    ...
    async function cargarPacienteSeleccionado(id_paciente) {
        try {
            if (!id_paciente) {
            ...
            const respuestaBackend = await res.json();
            const paciente = Array.isArray(respuestaBackend) ? respuestaBackend[0] : respuestaBackend;
            ...
            setRutPaciente(paciente.rut || "");
        } catch (error) {
            setPacienteSeleccionado(null);
            return toast.error("Error al cargar el paciente, contacte a soporte.");
        }
    }
    ...
    useEffect(() => {
        cargarPacienteSeleccionado(idPacienteParam);
    }, [idPacienteParam]);
    ...
    function limpiarDocumento() {
        ...
        router.replace("/dashboard/examenDocumento");
    }

    function volverFichaPaciente() {
        if (!pacienteSeleccionado?.id_paciente) return;
        router.push(`/dashboard/FichasPacientes/${pacienteSeleccionado.id_paciente}`);
    }
    ...
    async function generarDocumentoPDF() {
        ...
        doc.text(PDF_BRAND_TITLE, contentLeft, 23.5);

        doc.setFont("helvetica", "italic");
        doc.setFontSize(8.5);
        doc.setTextColor(92, 108, 128);
        doc.text(PDF_BRAND_SUBTITLE, contentLeft, 28.8);

        doc.setFont("helvetica", "normal");
        doc.setFontSize(8);
        doc.setTextColor(90, 90, 90);
        doc.text("Clinical exam request", contentLeft, 33);
        ...
        doc.text("EXAM REQUEST", contentRight, 22, {align: "right"});
        ...
        doc.text("Generated by Metabocare | Healthcare Information System", contentLeft, footerY);
        doc.text("Clinical exams", contentRight, footerY, {align: "right"});
                    ...
                    {pacienteSeleccionado?.id_paciente ? (
                        <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
                            <p className="text-sm text-slate-500">
                                Orden asociada al paciente ID #{pacienteSeleccionado.id_paciente}
                            </p>
                            <button
                                type="button"
                                onClick={volverFichaPaciente}
                                className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50 px-4 py-2.5 text-sm font-semibold text-violet-700 transition-all duration-150 hover:border-violet-300 hover:bg-violet-100"
                            >
