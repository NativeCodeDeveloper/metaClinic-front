...
export default function AgendaCitas() {
    ...
    useEffect(() => {
        ...
        if (nombrePacienteGuardado) {
            setnombrePaciente(nombrePacienteGuardado);
        }
    ...
    useEffect(() => {
        ...
        if (nombrePaciente.trim()) {
            window.localStorage.setItem(STORAGE_KEYS.nombrePaciente, nombrePaciente.trim());
        } else {
            window.localStorage.removeItem(STORAGE_KEYS.nombrePaciente);
        }
    }, [nombrePaciente]);
    ...
    useEffect(() => {
        ...
        if (rut.trim()) {
            window.localStorage.setItem(STORAGE_KEYS.rut, rut.trim());
        } else {
            window.localStorage.removeItem(STORAGE_KEYS.rut);
        }
    }, [rut]);
    ...
    async function crearPacienteDesdeReserva(reserva) {
        ...
        if (!rutNormalizado) {
            throw new Error("No se pudo normalizar el RUT del paciente");
        }
    ...
    function buscarPorRut(rut) {
        try {
            ...
            if (reservasFiltradas.length > 0) {
                return toast.success(`Se encontraron ${reservasFiltradas.length} reservas para el RUT ingresado.`);
            }
    ...
    function buscarPorNombres(nombrePaciente) {
        try {
            ...
            if (reservasFiltradas.length > 0) {
                return toast.success(`Se encontraron ${reservasFiltradas.length} reservas para el nombre ingresado.`);
            }
    ...
    async function actualizarEstadoReservaRapido(id_reserva, nuevoEstado) {
        try {
            ...
            if (respuestaBackend.message === true) {
                setDataListaBase((prev) => {
                    const nuevaBase = prev.map((item) => (
                        item.id_reserva === id_reserva
                            ? {...item, estadoReserva: nuevoEstado}
                            : item
                    ));
                    setdataLista(aplicarFiltrosCombinados(nuevaBase));
                    return nuevaBase;
                });
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-col gap-2 sm:flex-row">
                                                    <ShadcnInput
                                                        ...
                                                        onKeyDown={(e) => {
                                                            if (e.key === "Enter") {
                                                                buscarPorNombres(nombrePaciente);
                                                            }
                                                        }}
                                                        className="w-full"
                                                    />
                    ...
                    <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white/94 shadow-[0_16px_40px_rgba(15,23,42,0.06)]">
                        <div className="flex flex-col gap-4 border-b border-slate-100 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)] px-5 py-4 sm:px-6 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <div className="grid w-full grid-cols-1 gap-2 sm:grid-cols-3 lg:w-auto">
                                <Select
                                    value={id_profesional ? String(id_profesional) : SELECT_ALL_VALUE}
                                    onValueChange={(value) => setId_profesional(value === SELECT_ALL_VALUE ? "" : value)}
                                >
                                    ...
                                    <SelectContent className="z-50 border border-slate-200 bg-white shadow-lg">
                                        <SelectGroup>
                                            <SelectItem value={SELECT_ALL_VALUE}>Todos los profesionales</SelectItem>
                                ...
                                <Select
                                    value={estadoReserva || SELECT_ALL_VALUE}
                                    onValueChange={(value) => setestadoReserva(value === SELECT_ALL_VALUE ? "" : value)}
                                >
                                    <SelectTrigger className="h-10 w-full rounded-xl border-slate-200 bg-white text-xs text-slate-700 shadow-sm sm:w-[190px]">
                                    ...
                                    <SelectContent className="z-50 border border-slate-200 bg-white shadow-lg">
                                        <SelectGroup>
                                            <SelectItem value={SELECT_ALL_VALUE}>Todos los estados</SelectItem>
